...
Adapted for Claude Code from poteto's pstack `show-me-your-work` (cursor/plugins); kept as our own copy because the upstream audit and review steps assume Cursor transcripts and model routing.
...
At the end of the run, before handing back, check the log told the truth. Read this run's transcript: Claude Code stores it at `~/.claude/projects/<slug>/<session-uuid>.jsonl`, where `<slug>` is the working directory path with `/` replaced by `-` (so `/Users/you/proj` becomes `-Users-you-proj`). Pick the most recently modified file (`ls -t`) whose first user message matches this session. Don't glob across other projects' directories; that reads unrelated private chats. Walk the log against what actually happened:
...
Before handing back, you must spawn a fresh `Agent` (`general-purpose`, no shared context; use a different `model` tier than the one that did the work when one is available) to review the trail. Self-review is not a substitute; the point is fresh eyes you cannot bring yourself. The subagent reads the audit trail and the run's transcript, then flags what the user should pay attention to. Not a redo of the work, a scan for what's suboptimal or risky.
