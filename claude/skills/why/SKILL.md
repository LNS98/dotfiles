...
`why` answers what forces led to the code's shape, not what it does; for runtime behavior, walk the code (or use the `Explore` agent) instead.
...
If the target is vague ("why do we do it this way?" with no clear referent), make your best guess from conversation context (recent edits, files just read, what was just discussed). State your interpretation briefly so the user can redirect if you're off, then proceed.
...
Before spawning investigators, list the available MCP servers. In Claude Code, MCP tools are named `mcp__<server>__<tool>`; run `ToolSearch` with a keyword query (or check the deferred-tools list in the system prompt) to enumerate them. `claude mcp list` in the shell also shows configured servers.
...
- `subagent_type`: `general-purpose` (not `Explore`: investigators need MCP access, which they reach through `ToolSearch`; tell them the exact `mcp__<server>__*` tools to load)
- `model`: `sonnet` for cheap, fast grunt work; `opus` when the source needs judgment
- Investigators must not write anything. That's a posture, not a sandbox; say it in the prompt.
...
- `model`: omit (inherit the session model) so it gets the strongest available reasoning. The synthesizer's quality check spot-verifies citations, which can require MCP access.
